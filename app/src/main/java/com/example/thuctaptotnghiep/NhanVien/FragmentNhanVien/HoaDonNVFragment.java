...
import com.example.thuctaptotnghiep.NhanVien.AdapterNhanVien.HoaDonNVAdapter;
...
public class HoaDonNVFragment extends Fragment {
    List<HoaDonNV> donHangList;
    HoaDonNV donHang;
    HoaDonNVAdapter mAdapter;
    RecyclerView rvDanhSach;

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.nhanvien_fragment_quanly_hoadon,container,false);

        rvDanhSach = view.findViewById(R.id.rvDSHoaDon_NhanVien);

        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getContext(),RecyclerView.VERTICAL,false);
        rvDanhSach.setLayoutManager(linearLayoutManager);
        donHangList = new ArrayList<>();
        getHoaDon();
        mAdapter = new HoaDonNVAdapter(donHangList,getContext());
            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    for(int i=0;i<array.length();i++){
                        ...
                        if (nvGiaoHang == LoginActivity.user.getId()) {
                            donHang = new HoaDonNV(id, img, tenSp, slMua, hoTen, sdt, diaChiNhan, tongTien, ngayDat, xacNhan, nvGiaoHang, daNhan);
